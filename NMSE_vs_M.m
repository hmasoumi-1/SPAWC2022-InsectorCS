...
for itr_res = 1:length(Over_sampling)
    ...
    for itr_m = 1:length(MM)
        ...
        nmse_temp2_InSectorPaper(itr_m,itr_res) = mean(channel_est_err_InSectorPaper)/mean(h_normh_masked);
        RateInSector(itr_m,itr_res) = mean(Rate_InSector);
        
        nmse_temp2_SIb(itr_m,itr_res) = mean(channel_est_err_SIb)/mean(h_normh_masked);
        RateSIb(itr_m,itr_res) = mean(Rate_SIb);
        
        nmse_temp2_SIbrnd(itr_m,itr_res) = mean(channel_est_err_SIbrnd)/mean(h_normh_masked);
        RateSIbrnd(itr_m,itr_res) = mean(Rate_SIbrnd);
        
        nmse_temp2_Greedy(itr_m,itr_res) = mean(channel_est_err_Greedy)/mean(h_normh_masked);
        RateGreedy(itr_m,itr_res) = mean(Rate_Greedy);
